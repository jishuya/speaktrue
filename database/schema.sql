...
DROP TABLE IF EXISTS recording_transcripts CASCADE;
...
CREATE TABLE sessions (
    ...
    session_type    VARCHAR(20) DEFAULT 'chat',    -- 'chat', 'recording'
...
CREATE TABLE recording_details (
    ...
    recorded_at     TIMESTAMP NOT NULL,      -- 실제 녹음 시각

    created_at      TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_recording_details_session ON recording_details(session_id);

-- =============================================
-- 9. 녹음 대화 내용 (Recording Transcripts)
-- STT 변환된 커플 대화 내용
-- =============================================
CREATE TABLE recording_transcripts (
    ...
    speaker         VARCHAR(20) NOT NULL,    -- 'me', 'partner'
    content         TEXT NOT NULL,           -- 대화 내용
    start_time      INTEGER NOT NULL,        -- 시작 시간 (초)
    end_time        INTEGER NOT NULL,        -- 종료 시간 (초)
    nvc_suggestion  TEXT,                    -- NVC 제안 (AI 분석 후)

    created_at      TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_recording_transcripts_session ON recording_transcripts(session_id, start_time);
